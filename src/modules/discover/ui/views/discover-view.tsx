                ...
                >
                  {selectedPhotos.map((photo) => (
                    <div key={photo.id} className="space-y-2">
                      <div className="flex items-center justify-center bg-gray-50 dark:bg-muted h-[80vh] p-10 relative">
                        {photo.type === "video" ? (
                          <Link href={`/blog/${photo.id}`} className="block relative">
                            <FramedPhoto
                              src={photo.url}
                              alt={photo.title}
                      ...
                      <div className="flex flex-col w-full items-center justify-center">
                        ...
                        {photo.type === "video" && (
                          <span className="text-xs text-primary">视频</span>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          <DrawerContent className="md:hidden inset-0 h-screen w-screen rounded-none">
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-6 hide-scrollbar">
                {selectedPhotos.map((photo) => (
                  <div key={photo.id} className="space-y-2">
                    <div className="flex items-center justify-center bg-gray-50 dark:bg-muted p-4 rounded-xl relative">
                      {photo.type === "video" ? (
                        <Link href={`/blog/${photo.id}`} className="block relative">
                          <FramedPhoto
                            src={photo.url}
