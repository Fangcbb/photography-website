...
// Helper to request via CORS proxy
// Try direct request first, fallback to corsproxy.io if CORS fails
export const fetchViaProxy = async (targetUrl: string, timeoutMs = 8000): Promise<any> => {
  let text: string;
  ...
  try {
    ...
    // 2. Direct request failed (likely CORS or timeout), try proxy
    console.warn("Direct fetch failed, trying corsproxy.io:", directError);
